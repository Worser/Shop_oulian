...
public class BannerController extends BaseController{
	...
	@RequestMapping(params = "getButtonBanners", method = RequestMethod.POST)
	public void getButtonBanners(MerchantsAccountBean merchantsAccountBean,BannerButtonBean bannerButtonBean,HttpServletRequest request,
		...
		WriteObject(response, bannerServiceC.getButtonBanners(bannerButtonBean));
	}
	
	@RequestMapping(params = "insertButtonBanner", method = RequestMethod.POST)
	public void insertButtonBanner(MerchantsAccountBean merchantsAccountBean,BannerButtonBean bannerButtonBean,HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if(!systemService.verToken(merchantsAccountBean)){
			WritePending(response, "token failed");
			return;
		}		
		int num=bannerServiceC.insertButtonBanner(bannerButtonBean);
		if(num>0){
	...
	@RequestMapping(params = "updateButtonBanner", method = RequestMethod.POST)
	public void updateButtonBanner(MerchantsAccountBean merchantsAccountBean,BannerButtonBean bannerButtonBean,HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if(!systemService.verToken(merchantsAccountBean)){
			WritePending(response, "token failed");
			return;
		}		
		int num=bannerServiceC.updateButtonBanner(bannerButtonBean);
		if(num>0){
	...
	@RequestMapping(params = "deleteButtonBanner", method = RequestMethod.POST)
	public void deleteButtonBanner(MerchantsAccountBean merchantsAccountBean,BannerButtonBean bannerButtonBean,HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		if(!systemService.verToken(merchantsAccountBean)){
			WritePending(response, "token failed");
			return;
		}		
		int num=bannerServiceC.deleteButtonBanner(bannerButtonBean);
		if(num>0){
	...
	public void insertBanner(MerchantsAccountBean merchantsAccountBean,BannerBean bannerBean,HttpServletRequest request,
		...
		if(num>0){
			WriteMsg(response, bannerBean.getBanner_id()+"");
		}else{
			WriteError(response, "添加失败");
		}
	}
