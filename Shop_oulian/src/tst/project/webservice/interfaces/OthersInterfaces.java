...
public class OthersInterfaces extends BaseController {
	...
	public void test1(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String param=request.getParameter("param");//"{\"com\":\"yuantong\", \"num\":\"12345678\",\"from\":\"广东深圳\",\"to\":\"北京朝阳\"}";
		
		System.out.println(param+key+customer);
		String sign=MD5Util.encode(param+key+customer);
		WriteObject(response, Kuaidi100Utils.getLogistics("3328385662669", "shentong"));
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(params = "sendContent")
	public void sendContent(HostBean hostBean,HttpServletRequest request
					,HttpServletResponse response) throws Exception {
		VerificationBean verificationBean = codeService.getVerificationSetting(new VerificationBean());
		VerificationCodeUtils.sendContent(verificationBean,"15026592839","亲爱的XXX会员，已有会员购买您的宝贝啦，请您及时登录ACH平台确认");					
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(params = "getHost")
	public void getHost(HostBean hostBean,HttpServletRequest request
					,HttpServletResponse response) throws Exception {
		WriteObject(response,othersService.getHost(hostBean.setHost_type("1")));
	}
	...
	public void getWxAutho(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	public void sendCode(CodeBean codeBean, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(codeBean.getCode_type().equals("forget_passwrod"))
		{
			MemberBean memberBean2=memberService.getMemberByMobile(new MemberBean()
			               .setMember_account(codeBean.getMobile()));
			if(memberBean2==null){
		...
		CodeBean codeBean2=codeService.getCodeBeanByMobile(codeBean.setCreate_time(TimeUtils.getCurrentTime("yyyy-MM-dd HH:mm:ss")));
		if (codeBean2 != null) {
			CodeBean codeBean1 = VerificationCodeUtils.sendCode(verificationBean,
					codeBean.setCode(codeBean2.getCode()).setCode_desc(verificationBean.getVerification_content().replace("[code]", codeBean2.getCode())));
			WriteMsg(response,codeBean2.getCode());
			return;
		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(params = "getHtmlByClass", method = RequestMethod.POST)
	public void getHtmlDetail(HtmlBean htmlBean, HttpServletRequest request, HttpServletResponse response) throws Exception {
		WriteObject(response, othersService.getHtmlByClass(htmlBean));
	}
