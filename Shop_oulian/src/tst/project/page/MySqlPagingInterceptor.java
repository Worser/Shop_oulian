...
public class MySqlPagingInterceptor extends PageInterceptor {
    ...
    protected String getSelectPagingSql(String targetSql, PageBean bounds) {
    	...
    	if(order_index!=-1){
    		...
    		str=str+" "+sql.substring(order_index, sql.length())+" limit "+(bounds.getPage()-1)*bounds.getLimit()+","+bounds.getLimit();
    	}else{   
	        str=str+sql;
	        str=str+"	limit "+(bounds.getPage()-1)*bounds.getLimit()+","+bounds.getLimit();
    	}
        return str;
    }
