...
public class InsertOrderForMember extends Thread
{
  private MemberBean memberBean = null;
  private List<OrderGoodsBean> orderGoodsBeans = null;

  public InsertOrderForMember(MemberBean memberBean, List<OrderGoodsBean> orderGoodsBeans) { this.memberBean = memberBean;
  ...
  public void run()
  {
    ...
    try
    {
      ...
      msg.setRecipient(Message.RecipientType.TO, new InternetAddress(this.memberBean.getEmail_account()));

      msg.setSubject(this.memberBean.getCompany_name() + ",订单提醒！");

      StringBuilder sbd = new StringBuilder();
      sbd.append("【ACH】亲爱的" + this.memberBean.getCompany_name() + "，您好!<br/>");
      sbd.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您购买的商品");
      for (int i = 0; i < this.orderGoodsBeans.size(); i++) {
        sbd.append("【" + ((OrderGoodsBean)this.orderGoodsBeans.get(i)).getGoods_name() + "】");
      }
      sbd.append("已经下单成功，正在等待卖家审核，我们将立刻通知卖家进行审核,还请您耐心等待！！<br/>如果卖家审核");
      sbd.append("通过后，我们会以短信的形式向您发送，您就可以进行120分钟内进行支付，然后坐等收货啦！！<br/><br/>");
      sbd.append("------------------------------------------------<br/>");
      ...
      Transport.send(msg);
      System.out.println("给" + this.memberBean.getEmail_account() + "发送邮件成功。");
