...
// Tree object
function dTree(objName, path, toObj) {
	...
	this.icon = {root:path + "base.gif", folder:path + "folder.gif", folderOpen:path + "folderopen.gif", node:path + "page.gif", empty:path + "empty.gif", line:path + "line.gif", join:path + "join.gif", joinBottom:path + "joinbottom.gif", plus:path + "plus.gif", plusBottom:path + "plusbottom.gif", minus:path + "minus.gif", minusBottom:path + "minusbottom.gif", nlPlus:path + "nolines_plus.gif", nlMinus:path + "nolines_minus.gif"};
	this.obj = objName;
	this.nowsel = null;
	this.toObj = toObj;
	this.aNodes = [];
...
dTree.prototype.indent = function (node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		//alert("-----"+this.aNodes+"------"+nodeId+"------"+node.pid+"-----"+node.name);
		str += "<input type='checkbox' style='width:15px;height:15px' value='"+node.id+"'/>";
	}
	return str;
};
...
dTree.prototype.create = function () {
	$(this.toObj).innerHTML = this.toString(); 	
}